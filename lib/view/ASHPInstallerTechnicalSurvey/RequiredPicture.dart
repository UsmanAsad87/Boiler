              ...
              InkWell(
                onTap: () async {
                  ...
                  if(imageFileList!.isNotEmpty) {
                    imgUrls=await ASHPMethods().uploadImages(imageFileList!);
                    print("urls"+imgUrls.toString());
                    model= model.copyWith(
                      images: imgUrls,
                    );
                    print(model.toJson().toString().substring(602,));
                    Provider.of<ASHPProvider>(context,listen: false).setASHPObject(model);
                    String res= await ASHPMethods().createASHPSurvey(context: context);
                    showToast(res);
